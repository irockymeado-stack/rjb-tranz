...
import SessionTimeoutCarousel from "@/components/SessionTimeoutCarousel";
...
function App() {
  ...
  // Mobile tab navigation handlers with enhanced touch support
  ...
  const handleTouchEnd = () => {
    ...
    const maxVerticalMovement = 100; // Allow some vertical movement
  ...
  const getFilteredRates = () => {
    ...
    // Apply sorting with USA pinned to top
    filtered.sort((a, b) => {
      if (!a || !b) return 0;
      
      // Pin USA (USD) to the top
      const aIsUSD = a.pair === 'USD' || a.pair === 'USD/USD';
      const bIsUSD = b.pair === 'USD' || b.pair === 'USD/USD';
      
      if (aIsUSD && !bIsUSD) return -1;
      if (!aIsUSD && bIsUSD) return 1;
      
      // Normal sorting for non-USD pairs
              ...
              {getFilteredRates().map((rate, index) => {
                ...
                return (
                    <Card
                      ...
                      className="currency-pair group overflow-hidden card-hover-glass card-ripple animate-card-entrance cursor-pointer transition-fast hover:shadow-2xl hover:scale-[1.015] hover:border-primary/40 bg-gradient-to-br from-card via-card to-muted/20 gpu-accelerated active:shadow-3xl active:scale-[1.01] p-3"
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex justify-between items-center p-2 bg-muted/30 rounded-lg group-hover:bg-primary/10 transition-fast">
                          ...
                          <span className="font-bold text-lg font-mono group-hover:text-primary transition-fast">
